...
/// <summary>
/// Löst die HCT-Gleichung (Farbton, Chroma, Ton nach sRGB); portiert aus <c>hct/hct_solver.ts</c>
/// (material-color-utilities, Apache-2.0). Deterministisch: dieselbe Eingabe ergibt auf jeder Plattform dieselbe Farbe.
/// </summary>
internal static class HctSolver
{
    ...
    /// <summary>Farbe mit gegebenem Farbton (Grad), Chroma und L*; bei nicht erreichbarem Chroma wird es maximiert.</summary>
    ...
    private static int FindResultByJ(double hueRadians, double chroma, double y)
    {
        ...
        for (var iterationRound = 0; iterationRound < 5; iterationRound++)
        {
            ...
            var linrgb = MathUtils.MatrixMultiply([rCScaled, gCScaled, bCScaled], LinrgbFromScaledDiscount);
